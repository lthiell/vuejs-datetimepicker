<template>
  <div :style='{width:width}' class="datetime-picker" v-on:click='calendarClicked($event)' v-on:blur='toggleCal'>
    <div>
      <input type='text' class="form-control" :readonly="readonly" id='tj-datetime-input' :required="required" :value="date"
             :name='name' v-on:click='toggleCal' autocomplete='off'/>
        ...
        <div :class='{noDisplay: hideDate}'>
          ...
          <div>
            <div class="week" v-for="(week, weekIndex) in weeks" :key="weekIndex">
              <span class="port" v-for="(day, dayIndex) in week" :key="dayIndex" v-on:click='setDay(weekIndex*7 + dayIndex, day)'
